...
- `backend/models.py`: `Role` enum is only `SUPERADMIN / ADMIN / STAFF`.
  `STAFF` is **never checked in the backend routers** — every `require_role`
  gates on `ADMIN` only (verified: ~51 calls, all `Role.ADMIN`, zero on
  `STAFF`), so the backend enforces no STAFF-vs-ADMIN distinction. Note this
  does **not** mean STAFF users see nothing: the *frontend* already gives them
  a reduced nav (see the `layout.tsx` note below). So today is a **2-tier,
  frontend-only** split (admin = everything, staff = operational pages), not
  the "ADMIN-or-nothing" it first looks like — and because the split is
  client-side only, a STAFF user could call an admin API directly and the
  backend wouldn't stop them. Real backend enforcement + 7 tiers instead of 2
...
- `frontend/src/app/dashboard/layout.tsx`: gating today is `isStaff`
  (`user.role === "staff"`) plus an `adminOnly: true` flag on some `navItems`.
  Staff see only the operational pages (POS, Kitchen, Orders, Menu, Stock,
  Bookings, Sales) and are **client-side redirected** off the `adminOnly`
  routes (Home, AI, Growth, ROI, AI-Ops). This 2-tier filter + redirect is
  what the 7-tier matrix replaces — extend it, don't assume the page starts
  from a blank slate. And remember it's client-side only, so the redirect is
  UX, not a security boundary (that has to come from the backend change).
