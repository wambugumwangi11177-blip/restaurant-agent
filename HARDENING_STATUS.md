...
## Notification delivery-health pass (2026-08-05)
Prompted by "make sure the notifications work" ahead of going out to sell.

**The gap:** the notification chain fails *silently by design*. `twilio_client.send()`
returns `{"status": "not_configured"}` and writes a log line when config is missing, and
every caller treats a send as best-effort — nothing raises. `/health` and `/health/db` both
stay green through a total notification outage. Nothing validated Twilio at boot.

**Worst case found:** `TWILIO_WHATSAPP_FROM` *defaults* to Twilio's shared public sandbox
number. A deploy that forgets to set it gets `sent` back from Twilio for every message
while reaching only handsets that texted the sandbox join code — and those opt-ins expire
after 72h. No error anywhere. Now detected and warned about explicitly.
...
- **`startup_checks`** warns loudly on notification config — deliberately SOFT even in
  production: a POS that refuses to boot because WhatsApp is misconfigured is worse for a
  restaurant than a late briefing.
- **`execution/verify_notifications.py`** — preflight before a demo or an onboarding.
  Read-only by default (safe against prod); `--send +2547…` puts one real message through
  to prove delivery; `--url` checks a deployed instance.
- Tests: `test_notifications_health.py` (15). Suite **377 passed**, Bandit clean at `-ll`,
  frontend `tsc --noEmit` clean, `next build` green (18 routes).

### An in-app notification feed was built here and withdrawn
This branch also implemented a full in-app feed (table, router, bell, audience routing,
per-category muting, activity recording). It was **removed before merge**:
...
depending on it. Keeping both would have produced two `notifications` tables, two routers,
two bells, and — the blocking problem — two Alembic revisions numbered 025 off 024, giving
multiple heads and an `alembic upgrade head` failure at container start (the Dockerfile
runs it, so the container would not boot).

**That branch's implementation wins.** Two findings from this one are worth porting onto
it and are tracked as **D18**: alert bodies carry revenue (the morning briefing has
yesterday's takings, WoW movement and payment split), so the feed needs role scoping or
STAFF read it; and once routine activity is recorded, the bell needs per-category muting
with stock-critical/depleted deliberately unmutable. The withdrawn implementation is in
this branch's history at `aa00e53` and `f532eda` if it is wanted as a reference.

## Go-to-market (2026-08-05)
...
- **`execution/outreach_pipeline.py`** — deterministic lead pipeline (add / log / stage /
  today / show / stats). Funnel conversion is computed on the deepest stage each lead ever
  reached, so a lost deal stays in the demo denominator instead of quietly inflating the
  close rate. Data lands in `outreach/`, self-gitignored — it holds real prospects' phone
