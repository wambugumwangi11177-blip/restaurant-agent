...
- [x] ~~Set/verify prod env on Railway: `APP_ENV`, `CORS_ORIGINS`~~ **VERIFIED 2026-09-20** — both are set on the `backend-api` service. Confirmed from the production startup log of the last successful deploy rather than from the variable list (Railway returns names only, not values, to a connected app): the `prod`-gated warnings in `startup_checks.collect_problems()` fire ("SMTP is not configured **in production**", "SENTRY_DSN is not set **in production**", "STORAGE_BACKEND=local **in production**"), which is only reachable when `is_production()` is true — so `APP_ENV=production` is really in effect. The `CORS_ORIGINS is not set` warning is absent from that same log, which is likewise only possible when it IS set.
- [ ] `MPESA_CALLBACK_TOKEN` — still unset, and correctly a soft warning rather than a boot failure: `_mpesa_in_use()` is false because the service has no `MPESA_ENV` and no Daraja credentials. `_verify_mpesa_token()` rejects every callback with 403 while it is unset, so the callback is fail-closed, not forgeable. This becomes a hard blocker the moment M-Pesa is switched on.
- [ ] **Sentry may not actually be reporting — recheck.** The production startup log for the last successful deploy (2026-09-19, commit 82754b5) says `SENTRY_DSN is not set in production`. That check is `not os.getenv("SENTRY_DSN", "").strip()`, so a variable that exists but is EMPTY trips it — and `SENTRY_DSN` does appear in the Railway variable list. Either it is set to an empty value, or it was added after that deploy and has not been redeployed since. Until a boot log stops emitting that warning, treat production as running blind to unhandled exceptions. The note below records the original setup, which was real; what is unverified is whether the DSN is still live on the deployed service.
