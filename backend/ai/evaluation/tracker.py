...
def get_ai_ops_summary(db: Session, restaurant_id: int, days: int = 30) -> dict:
    ...
    return {
        ...
        "llm": {
            ...
            "total_cost_kes": round(total_cost_usd * USD_TO_KES, 2),
            "cost_per_response_usd": cost_per_response,
        },
        "agents": agents,
        "grounding": grounding,
        "roi": {
            "money_captured_cents": money_captured_cents,
            "money_captured_usd": round(money_captured_cents / 100, 2),
            "ai_cost_usd": total_cost_usd,
            "roi_ratio": round((money_captured_cents / 100) / total_cost_usd, 2) if total_cost_usd else None,
        },
    }