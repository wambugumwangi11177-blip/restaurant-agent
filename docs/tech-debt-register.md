...
| D15 | eslint toolchain inherits a HIGH advisory | P3 | `eslint`, `@typescript-eslint/*`, `eslint-config-next` and `eslint-plugin-*` all inherit one HIGH from `minimatch`/`brace-expansion`. All are **devDependencies** — they run in CI and on developer machines and are never served to a user, so they cannot affect the deployed app. Not fixed because the two available routes are both worse than the debt: a major `eslint` v10 upgrade (deliberate, separate work), or forcing `minimatch` v10 into plugins written against v3, which risks silent glob-matching breakage in linting. The CI gate is scoped `--omit=dev` so production advisories still block; re-check when eslint v10 is adopted. | `.github/workflows/ci.yml`; `frontend/package.json` |
| D16 | Frontend `no-explicit-any` lint debt | P3 | `npm run lint` reports ~57 `@typescript-eslint/no-explicit-any` errors across 26 files. Pre-existing and codebase-wide, not a regression. `lint` is deliberately **not** a CI gate (frontend-ci runs `npm ci`, `npm audit`, `npm run build`, `npx tsc --noEmit`); typing these properly is its own pass. | `frontend/src/**` |
...
| 1.2 | 2026-07-28 | Engineering | Add D15 (eslint toolchain advisory, dev-only) and D16 (`no-explicit-any` lint debt) after the npm-audit triage |
