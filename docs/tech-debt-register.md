...
| D16 | Notification feed has no retention policy | **P2** | `notifications` is append-only and now records routine activity too (every order, status change, payment, booking), so a busy site writes hundreds of rows a day. Needs a purge/archive job, and per-category muting so the bell stays readable | `backend/notifications.py` (activity feed section) |
