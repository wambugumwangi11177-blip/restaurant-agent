...
def verify():
    print("Starting backend server for AI verification...", flush=True)
    if os.name == 'nt':
        uvicorn_cmd = os.path.abspath(os.path.join("backend", "venv", "Scripts", "uvicorn"))
    else:
        uvicorn_cmd = os.path.abspath(os.path.join("backend", "venv", "bin", "uvicorn"))
        
    process = subprocess.Popen(
        [uvicorn_cmd, "main:app", "--port", "8005"],
        cwd="backend",
