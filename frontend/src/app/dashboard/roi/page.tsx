...
import { Clock, TrendingUp, Target, RefreshCw, AlertTriangle, Zap, ChefHat, Scale, CalendarClock } from "lucide-react";
import EmptyState from "@/components/ui/EmptyState";
import { getErrorMessage } from "@/lib/errors";

const TrendChip = dynamic(() => import("./_components/TrendChip"));
const RoiBreakdown = dynamic(() => import("./_components/RoiBreakdown"));
...
export default function RoiDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setError(getErrorMessage(e, "Could not load ROI data"));
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="bg-surface rounded-xl h-32 animate-pulse" />
                    ))}
                </div>
            </div>
        );
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
                ...
                <p className="text-text font-medium">Could not load ROI data</p>
    ...
    if (!data) {
        return (
            <EmptyState
                pageTitle="Time & Money Saved"
                pageSubtitle="Getting started"
                icon={Clock}
                title="No automation activity yet"
                description="Once the AI starts sending WhatsApp messages, running analysis, or you approve a pricing recommendation, this page will show exactly how many hours and how much money it's saved you — and how much it's saving you each day."
            />
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-start justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-text">Time & Money Saved</h1>
            ...
            {isSparse && (
                <div className="rounded-xl border border-[var(--accent)]/25 bg-[var(--accent)]/[0.06] p-4 flex items-start gap-3">
                    ...
                    <div>
                        <p className="text-sm font-medium text-text">Your ROI dashboard is still gathering data</p>
            ...
            <div className="rounded-xl border border-[var(--accent)]/25 bg-[var(--accent)]/[0.04] p-5">
                <div className="flex items-center gap-2 text-[var(--accent)] mb-1">
                    ...
                    <p className="text-sm font-semibold text-text">How your AI is paying for itself</p>
                </div>
                <p className="text-sm text-[#a3a3a3] leading-relaxed">
                    ...
                    earn more. Below are the <span className="text-text font-medium">three separate ways</span> that
            ...
            <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/[0.05] p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    ...
                    <p className="text-3xl font-black text-text">
                        {hoursPerDay} hrs <span className="text-text-dim text-xl font-bold">·</span> {formatKES(moneyPerDayCents)}
                    </p>
                </div>
                <p className="text-text-muted text-xs max-w-xs sm:text-right">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-6">
                    ...
                    <p className="text-3xl font-black text-text">{ts.hours_saved_30d} hrs</p>
                ...
                <div className="rounded-xl border border-[var(--accent)]/30 bg-[var(--accent)]/5 p-6">
                    ...
                    <p className="text-3xl font-black text-text">{formatKES(mc.monthly_impact_cents)}</p>
                    <p className="text-text-dim text-sm mt-1">
                    ...
                    {!!mc.all_time_approved && mc.all_time_monthly_impact_cents !== undefined && (
                        <p className="text-[11px] text-text-dim mt-2 pt-2 border-t border-[var(--accent)]/15">
                ...
                <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-6">
                    ...
                    <p className="text-3xl font-black text-text">{formatKES(oppsTotal)}</p>
                    <p className="text-text-dim text-sm mt-1">not yet acted on</p>
            ...
            {econ && econ.roi_multiple !== null && econ.roi_multiple !== undefined && (
                <div className="rounded-xl border border-[var(--accent)]/30 bg-gradient-to-br from-[var(--accent)]/[0.07] to-transparent p-5">
                    ...
                    <div className="flex flex-wrap items-end gap-x-4 gap-y-1">
                        <p className="text-3xl font-black text-text">
                            {econ.roi_multiple}× <span className="text-base font-bold text-text-muted">return</span>
                        </p>
                        <p className="text-[#a3a3a3] text-sm mb-1">
                            Every <span className="text-text font-medium">{formatKES(100)}</span> spent on AI returned{" "}
            ...
            <RoiBreakdown breakdown={ts.breakdown} hoursSaved30d={ts.hours_saved_30d} />

            {/* Opportunities breakdown */}
            {opps.length > 0 && (
                <div className="rounded-xl border border-surface-hover bg-[#0f0f0f] p-5">
                    <p className="text-text font-semibold text-sm mb-1">Money on the table the AI has found</p>
                    <p className="text-text-dim text-xs mb-4">Flagged but not yet acted on — approve or action these to capture it.</p>
                    <div className="space-y-3">
                        {opps.map((o, i) => (
                            <div key={i} className="py-2 border-b border-surface-hover last:border-0">
                                ...
                                {OPP_SOURCE_EXPLAIN[o.source] && (
                                    <p className="text-[11px] text-text-dim mt-1">{OPP_SOURCE_EXPLAIN[o.source]}</p>
                                )}
                            </div>
                        ))}
                    </div>
            ...
            {cap && (
                <div className="rounded-xl border border-surface-hover bg-[#0f0f0f] p-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                        <div>
                            <p className="text-2xl font-bold text-text">{cap.avg_order_minutes}<span className="text-sm text-text-dim"> min</span></p>
                            <p className="text-text-dim text-xs mt-1">avg order time</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-text">{cap.orders_per_day}</p>
                            <p className="text-text-dim text-xs mt-1">orders / day</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-text">{cap.bottlenecks_found}</p>
                            <p className="text-text-dim text-xs mt-1">bottlenecks flagged</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-text">{cap.reclaimable_delay_minutes}<span className="text-sm text-text-dim"> min</span></p>
                            <p className="text-text-dim text-xs mt-1">reclaimable delay</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {proj && (proj.annual_money_saved_cents > 0 || proj.annual_captured_cents > 0) && (
                <div className="rounded-xl border border-surface-hover bg-[#0f0f0f] p-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <p className="text-2xl font-bold text-text">{proj.annual_hours_saved.toLocaleString()}<span className="text-sm text-text-dim"> hrs</span></p>
                            <p className="text-text-dim text-xs mt-1">staff time saved / year</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-emerald-400">{formatKES(proj.annual_money_saved_cents)}</p>
                            <p className="text-text-dim text-xs mt-1">labour value / year</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-[var(--accent)]">{formatKES(proj.annual_captured_cents)}</p>
                            <p className="text-text-dim text-xs mt-1">captured profit / year</p>
                        </div>
                    </div>
                </div>
            )}
