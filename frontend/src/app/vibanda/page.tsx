"use client";
// Vibanda Village — HOME = "Overview Today".
// Heading identity per user: "Overview Today" (greeting kept as a small line).
// Sections: 6 KPI cards → What needs your attention → What's happening (pulse)
// → How each part is doing → Business performance (+ drafted daily report).
...
export default function VibandaHomePage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      {/* Heading identity: Overview Today */}
      <section>
        <p className="text-sm text-[var(--muted-foreground)]">
          {greetingNow()}{firstName ? `, ${firstName}` : ""} · here&apos;s where things stand.
        </p>
        <h1 className="text-2xl md:text-3xl font-semibold mt-1">Overview Today</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-xs text-[var(--muted-foreground)]">Prototype data · live POS sync coming soon</span>
      </div>
        ...
        <>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Card label="Stock" headline={feed.stock.low_stock.length ? `${feed.stock.low_stock.length} items to watch` : "Stock looks healthy"} verdict="Stock"
              substats={[feed.stock.low_stock[0] ? `${feed.stock.low_stock[0].name} projected to run out` : "Nothing below reorder point"]} />
              ...
              {feed.attention.map((c) => (
                <div key={c.id} className="rounded-2xl border border-[var(--border)] p-4">
                  ...
                  <div className="flex gap-2 mt-3 flex-wrap">
                    ...
                    <button onClick={() => { window.location.assign(`/vibanda/os?q=${encodeURIComponent(c.title)}`); }}
                      className="rounded-full px-3 py-1.5 text-sm border border-[var(--border)] text-[var(--accent)]">Ask AI about this</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Business performance + drafted daily report */}
          <section>
            ...
            {dailyReport && (
              <div className="mt-3 rounded-2xl border border-[var(--border)]">
                <button onClick={() => setShowReport((s) => !s)}
                  className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium">
                  <span>Daily report (drafted from your data)</span>
                  <span className="text-[var(--muted-foreground)]">{showReport ? "Hide" : "Show"}</span>
                </button>
                {showReport && (
                  <pre className="whitespace-pre-wrap text-sm font-sans px-4 pb-4">{dailyReport}</pre>
                )}
              </div>
            )}
          </section>
